use crate::application::auth::registration::{
    contract::RegisterContract, data::RequestUserData
};
...
pub async fn handle_create<T: RegisterContract>(
    service: web::Data<T>,
    ...
    // create user and action token. Return action token so id can be set to header
    let action_token = service.create(data, &base_url).await?;
    //create response
